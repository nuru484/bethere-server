...
const uiOptions = {
  customSiteTitle: "BeThere API reference",
  ...
  swaggerOptions: {
    ...
    // 90 operations expanded on load is unreadable; show the tag list and let
    // the reader open what they came for.
    docExpansion: "none",
    filter: true,
...
export function mountApiDocs(app) {
  ...
  app.get(`${DOCS_PATH}.json`, (req, res) => {
